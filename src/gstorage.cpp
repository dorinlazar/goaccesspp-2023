...
static GParse paneling[] = {
    ...
    {
        MIME_TYPE, gen_mime_type_key, insert_data, insert_rootmap, insert_hit, insert_visitor, insert_bw, insert_cumts,
        insert_maxts, NULL, /*method*/
        NULL,               /*protocol*/
        NULL,               /*agent*/
    },
    {
...
static GParse* panel_lookup(GModule module) {
...
static void count_bw(int numdate, uint64_t resp_size) { ht_inc_cnt_bw(numdate, resp_size); }

/* Keep track of all invalid log strings. */
static void count_invalid(GLog* glog, const char* line) {
  glog->invalid++;
...
static void count_valid(int numdate) {
  lock_spinner();
...
static int insert_dkeymap(GModule module, GKeyData* kdata) {
 ...
 * On error, 0 is returned.
 * On success the value of the key inserted is returned */
static int insert_rkeymap(GModule module, GKeyData* kdata) {
 ...
 * On error, 0 is returned.
 * On success the value of the key inserted is returned */
static int insert_uniqmap(GModule module, GKeyData* kdata, uint32_t uniq_nkey) {
...
static void insert_root(GModule module, GKeyData* kdata) {
  ht_insert_root(module, kdata->numdate, kdata->data_nkey, kdata->root_nkey, kdata->cdnkey, kdata->crnkey);
}
...
static char* gen_unique_req_key(GLogItem* logitem) {
  char* key = NULL;
 ...
 * data structure */
static void get_kdata(GKeyData* kdata, const char* data_key, const char* data) {
  /* inserted in datamap */
  kdata->data = data;
  /* inserted in keymap */
...
/* A wrapper to assign the given data key and the data item to the key
 * data structure */
static void get_kroot(GKeyData* kdata, const char* root_key, const char* root) {
  /* inserted in datamap */
  kdata->root = root;
  /* inserted in keymap */
...
static void set_spec_visitor_key(char** fdate, const char* ftime) {
 ...
 * structure.
 */
static int gen_req_key(GKeyData* kdata, GLogItem* logitem) {
  if (!logitem->req)
    return 1;
...
static const char* extract_mimemajor(const char* token) {
  const char* lookfor;

  /* official IANA registries as per https://www.iana.org/assignments/media-types/ */

  if ((lookfor = "application", !strncmp(token, lookfor, 11)) || (lookfor = "audio", !strncmp(token, lookfor, 5)) ||
      (lookfor = "font", !strncmp(token, lookfor, 4)) ||
      /* unlikely */
      (lookfor = "example", !strncmp(token, lookfor, 7)) || (lookfor = "image", !strncmp(token, lookfor, 5)) ||
      /* unlikely */
      (lookfor = "message", !strncmp(token, lookfor, 7)) || (lookfor = "model", !strncmp(token, lookfor, 5)) ||
      (lookfor = "multipart", !strncmp(token, lookfor, 9)) || (lookfor = "text", !strncmp(token, lookfor, 4)) ||
 ...
 * If not valid, NULL is returned.
 * If valid, the appropriate constant string is returned. */
static const char* extract_tlsmajor(const char* token) {
  const char* lookfor;

  if ((lookfor = "SSLv3", !strncmp(token, lookfor, 5)) || (lookfor = "TLSv1.1", !strncmp(token, lookfor, 7)) ||
      (lookfor = "TLSv1.2", !strncmp(token, lookfor, 7)) || (lookfor = "TLSv1.3", !strncmp(token, lookfor, 7)) ||
      (lookfor = "TLS1.1", !strncmp(token, lookfor, 6)) || (lookfor = "TLS1.2", !strncmp(token, lookfor, 6)) ||
      (lookfor = "TLS1.3", !strncmp(token, lookfor, 6)) ||
      /* Nope, it's not 1.0 */
      (lookfor = "TLSv1", !strncmp(token, lookfor, 5)) || (lookfor = "TLS1", !strncmp(token, lookfor, 4)))
    return lookfor;
  return NULL;
}
 ...
 * returned. */
static int extract_geolocation(GLogItem* logitem, char* continent, char* country) {
...
static void parse_time_specificity_string(char* hmark, char* ftime) {
...
static int include_uniq(GLogItem* logitem) {
...
static void set_datamap(GLogItem* logitem, GKeyData* kdata, const GParse* parse) {
  GModule module;
  module = parse->module;
...
static void map_log(GLogItem* logitem, const GParse* parse, GModule module) {
  GKeyData kdata;
...
static void ins_agent_key_val(GLogItem* logitem, uint32_t numdate) {
...
static int clean_old_data_by_date(uint32_t numdate) {
  uint32_t* dates = NULL;
  uint32_t idx, len = 0;
