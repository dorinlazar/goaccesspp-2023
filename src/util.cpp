...
static const char* code_type[][2] = {
    {"1", STATUS_CODE_1XX}, {"2", STATUS_CODE_2XX}, {"3", STATUS_CODE_3XX},
    {"4", STATUS_CODE_4XX}, {"5", STATUS_CODE_5XX},
};

/* HTTP status codes */
static const char* codes[][2] = {
    {"100", STATUS_CODE_100}, {"101", STATUS_CODE_101}, {"200", STATUS_CODE_200}, {"201", STATUS_CODE_201},
    {"202", STATUS_CODE_202}, {"203", STATUS_CODE_203}, {"204", STATUS_CODE_204}, {"205", STATUS_CODE_205},
    {"206", STATUS_CODE_206}, {"207", STATUS_CODE_207}, {"208", STATUS_CODE_208}, {"300", STATUS_CODE_300},
    {"301", STATUS_CODE_301}, {"302", STATUS_CODE_302}, {"303", STATUS_CODE_303}, {"304", STATUS_CODE_304},
    {"305", STATUS_CODE_305}, {"307", STATUS_CODE_307}, {"308", STATUS_CODE_308}, {"400", STATUS_CODE_400},
    {"401", STATUS_CODE_401}, {"402", STATUS_CODE_402}, {"403", STATUS_CODE_403}, {"404", STATUS_CODE_404},
    {"405", STATUS_CODE_405}, {"406", STATUS_CODE_406}, {"407", STATUS_CODE_407}, {"408", STATUS_CODE_408},
    {"409", STATUS_CODE_409}, {"410", STATUS_CODE_410}, {"411", STATUS_CODE_411}, {"412", STATUS_CODE_412},
    {"413", STATUS_CODE_413}, {"414", STATUS_CODE_414}, {"415", STATUS_CODE_415}, {"416", STATUS_CODE_416},
    {"417", STATUS_CODE_417}, {"418", STATUS_CODE_418}, {"421", STATUS_CODE_421}, {"422", STATUS_CODE_422},
    {"423", STATUS_CODE_423}, {"424", STATUS_CODE_424}, {"426", STATUS_CODE_426}, {"428", STATUS_CODE_428},
    {"429", STATUS_CODE_429}, {"431", STATUS_CODE_431}, {"444", STATUS_CODE_444}, {"451", STATUS_CODE_451},
    {"494", STATUS_CODE_494}, {"495", STATUS_CODE_495}, {"496", STATUS_CODE_496}, {"497", STATUS_CODE_497},
    {"499", STATUS_CODE_499}, {"500", STATUS_CODE_500}, {"501", STATUS_CODE_501}, {"502", STATUS_CODE_502},
    {"503", STATUS_CODE_503}, {"504", STATUS_CODE_504}, {"505", STATUS_CODE_505}, {"520", STATUS_CODE_520},
    {"521", STATUS_CODE_521}, {"522", STATUS_CODE_522}, {"523", STATUS_CODE_523}, {"524", STATUS_CODE_524}};
...
char* substring(const char* str, int begin, int len) {
  ...
  if ((buffer = cppalloc<char>(len + 1)) == NULL)
    return NULL;
...
char* alloc_string(const char* str) {
  char* n = cppalloc<char>(strlen(str) + 1);
  strcpy(n, str);
  return n;
}
...
static int wc_match(const char* wc, char* str) {
  while (*wc && *str) {
    if (*wc == '*') {
      while (*wc && *wc == '*')
        wc++;
...
static int within_range(const char* ip, const char* start, const char* end) {
  ...
  if (1 == inet_pton(AF_INET, ip, &addr4)) {
    ...
    if (1 != inet_pton(AF_INET, end, &end4))
      return 0;
    if (memcmp(&addr4, &start4, sizeof(addr4)) >= 0 && memcmp(&addr4, &end4, sizeof(addr4)) <= 0)
      return 1;
  }
  /* IPv6 */
    ...
    if (1 != inet_pton(AF_INET6, end, &end6))
      return 0;
    if (memcmp(&addr6, &start6, sizeof(addr6)) >= 0 && memcmp(&addr6, &end6, sizeof(addr6)) <= 0)
      return 1;
  }
...
char* trim_str(char* str) { return rtrim(ltrim(str)); }
...
char* filesize_str(unsigned long long log_size) {
  char* size = cppalloc<char>(sizeof(char) * 12);
  if (log_size >= (1ULL << 50))
...
char* usecs_to_str(unsigned long long usec) {
  char* size = cppalloc<char>(sizeof(char) * 11);
  if (usec >= DAY)
...
char* int2str(int d, int width) {
  char* s = cppalloc<char>(snprintf(NULL, 0, "%*d", width, d) + 1);
  sprintf(s, "%*d", width, d);
...
char* u322str(uint32_t d, int width) {
  char* s = cppalloc<char>(snprintf(NULL, 0, "%*u", width, d) + 1);
  sprintf(s, "%*u", width, d);
...
char* u642str(uint64_t d, int width) {
  char* s = cppalloc<char>(snprintf(NULL, 0, "%*" PRIu64, width, d) + 1);
...
char* float2str(float d, int width) {
  char* s = cppalloc<char>(snprintf(NULL, 0, "%*.2f", width, d) + 1);
...
char* left_pad_str(const char* s, int indent) {
  ...
  buf = cppalloc<char>(snprintf(NULL, 0, "%*s", indent, s) + 1);
...
char* unescape_str(const char* src) {
  ...
  dest = cppalloc<char>(strlen(src) + 1);
  q = dest;
