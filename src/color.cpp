...
static const char* nocolors[] = {
    ...
    "COLOR_MTRC_MAXTS             color0:color-1",         "COLOR_MTRC_PROT              color0:color-1",
    "COLOR_MTRC_MTHD              color0:color-1",         "COLOR_MTRC_HITS_PERC         color0:color-1",
    "COLOR_MTRC_HITS_PERC_MAX     color0:color-1",         "COLOR_MTRC_VISITORS_PERC     color0:color-1",
    "COLOR_MTRC_VISITORS_PERC_MAX color0:color-1",         "COLOR_PANEL_COLS             color0:color-1",
    "COLOR_BARS                   color0:color-1",         "COLOR_ERROR                  color0:color-1",
    "COLOR_SELECTED               color0:color-1 reverse", "COLOR_PANEL_ACTIVE           color0:color-1 reverse",
    "COLOR_PANEL_HEADER           color0:color-1 reverse", "COLOR_PANEL_DESC             color0:color-1",
    "COLOR_OVERALL_LBLS           color0:color-1",         "COLOR_OVERALL_VALS           color0:color-1",
    "COLOR_OVERALL_PATH           color0:color-1",         "COLOR_ACTIVE_LABEL           color0:color-1 reverse",
...
static int get_color_item_enum(const char* str) { return str2enum(CSTM_COLORS, ARRAY_SIZE(CSTM_COLORS), str); }
...
static int extract_color(char* color) {
  char* sEnd;
  int col = 0;
...
static void locate_attr_color(GColors* color, const char* attr) {
  if (strstr(attr, "bold"))
    color->attr |= A_BOLD;
 ...
 * On error, 1 is returned.
 * On success, 0 is returned. */
static int parse_attr_color(GColors* color, const char* value) {
  char *line, *ptr, *start;
  int ret = 0;
 ...
 * On error, 1 is returned.
 * On success, 0 is returned. */
static int parse_module_color(GColors* color, const char* value) {
...
static int find_color_in_list(void* data, void* color) {
  GColors* new_color = static_cast<GColors*>(color);
  GColors* old_color = static_cast<GColors*>(data);
 ...
 * If color exists, 1 is returned.
 * If color does not exist, 1 is returned. */
static int find_pair_in_list(void* data, void* color) {
  GColorPair* new_color = static_cast<GColorPair*>(color);
  GColorPair* old_color = static_cast<GColorPair*>(data);
...
static int find_color_item_in_list(void* data, void* needle) {
  GColors* color = static_cast<GColors*>(data);
  GColorItem* item = static_cast<GColorItem*>(needle);
 ...
 * If the items match and with no module, 1 is returned.
 * If condition is not satisfied, 0 is returned. */
static int find_color_item_module_in_list(void* data, void* needle) {
  GColors* color = static_cast<GColors*>(data);
  GColors* item = static_cast<GColors*>(needle);
...
GColors* get_color_by_item_module(GColorItem item, GModule module) {
  ...
  if ((match = list_find(color_list, find_color_item_module_in_list, needle)))
    color = static_cast<GColors*>(match->data);
...
static void parse_color_line(GColorPair* pair, GColors* color, char* line) {
  ...
  color->item = static_cast<GColorItem>(item);
}
...
static void prepend_color(GColors** color) {
