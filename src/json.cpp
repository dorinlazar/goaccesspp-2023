...
static void free_json(GJSON* json) {
  if (!json)
    return;
...
void set_json_nlines(int newline) { nlines = newline; }

/* Make sure that we have enough storage to write "len" bytes at the
 * current offset. */
static void set_json_buffer(GJSON* json, int len) {
  char* tmp = NULL;
...
__attribute__((format(printf, 2, 3))) static void pjson(GJSON* json, const char* fmt, ...) {
  int len = 0;
  va_list args;
...
static void escape_json_other(GJSON* json, const char** s) {
...
static void escape_json_output(GJSON* json, const char* s) {
  while (*s) {
    switch (*s) {
      /* These are required JSON special characters that need to be escaped. */
    case '"': pjson(json, "\\\""); break;
    case '\\': pjson(json, "\\\\"); break;
    case '\b': pjson(json, "\\b"); break;
    case '\f': pjson(json, "\\f"); break;
    case '\n': pjson(json, "\\n"); break;
    case '\r': pjson(json, "\\r"); break;
    case '\t': pjson(json, "\\t"); break;
    case '/': pjson(json, "\\/"); break;
    default: escape_json_other(json, &s); break;
    }
    s++;
  }
}
...
static void pskeysval(GJSON* json, const char* key, const char* val, int sp, int last) {
  if (!last)
...
static void pskeyu64val(GJSON* json, const char* key, uint64_t val, int sp, int last) {
  if (!last)
...
static void pskeyfval(GJSON* json, const char* key, float val, int sp, int last) {
  if (!last)
...
static void popen_obj(GJSON* json, int iisp) {
...
static void popen_obj_attr(GJSON* json, const char* attr, int sp) {
...
/* Close JSON object. */
static void pclose_obj(GJSON* json, int iisp, int last) {
  if (!last)
...
static void popen_arr_attr(GJSON* json, const char* attr, int sp) {
  /* open object attribute */
  pjson(json, "%.*s\"%s\": [%.*s", sp, TAB, attr, nlines, NL);
}
...
static void pclose_arr(GJSON* json, int sp, int last) {
  if (!last)
...
static void poverall_datetime(GJSON* json, int sp) {
  char now[DATE_TIME];
...
/* Write to a buffer the date and time for the overall object. */
static void poverall_start_end_date(GJSON* json, GHolder* h, int sp) {
...
static void poverall_requests(GJSON* json, int sp) { pskeyu64val(json, OVERALL_REQ, ht_get_processed(), sp, 0); }

/* Write to a buffer the number of valid requests under the overall
 * object. */
static void poverall_valid_reqs(GJSON* json, int sp) { pskeyu64val(json, OVERALL_VALID, ht_sum_valid(), sp, 0); }

/* Write to a buffer the number of invalid requests under the overall
 * object. */
static void poverall_invalid_reqs(GJSON* json, int sp) { pskeyu64val(json, OVERALL_FAILED, ht_get_invalid(), sp, 0); }

/* Write to a buffer the total processed time under the overall
 * object. */
static void poverall_processed_time(GJSON* json, int sp) {
 ...
 * overall object. */
static void poverall_visitors(GJSON* json, int sp) {
 ...
 * overall object. */
static void poverall_files(GJSON* json, int sp) {
 ...
 * overall object. */
static void poverall_excluded(GJSON* json, int sp) {
...
static void poverall_refs(GJSON* json, int sp) {
 ...
 * object. */
static void poverall_notfound(GJSON* json, int sp) {
 ...
 * the overall object. */
static void poverall_static_files(GJSON* json, int sp) {
 ...
 * overall object. */
static void poverall_log_size(GJSON* json, int sp) {
  pjson(json, "%.*s\"%s\": %jd,%.*s", sp, TAB, OVERALL_LOGSIZE, (intmax_t)get_log_sizes(), nlines, NL);
}
...
static void phits(GJSON* json, GMetrics* nmetrics, int sp) {
  int isp = 0;
...
static void pvisitors(GJSON* json, GMetrics* nmetrics, int sp) {
  int isp = 0;
...
static void pbw(GJSON* json, GMetrics* nmetrics, int sp) {
  int isp = 0;
...
static void pavgts(GJSON* json, GMetrics* nmetrics, int sp) {
  if (!conf.serve_usecs)
    return;
...
static void pcumts(GJSON* json, GMetrics* nmetrics, int sp) {
  if (!conf.serve_usecs)
    return;
...
static void pmaxts(GJSON* json, GMetrics* nmetrics, int sp) {
  if (!conf.serve_usecs)
    return;
...
static void pmethod(GJSON* json, GMetrics* nmetrics, int sp) {
  /* request method */
...
static void pprotocol(GJSON* json, GMetrics* nmetrics, int sp) {
  /* request protocol */
...
static void pmeta_i64_data(GJSON* json, GHolder* h, void (*cb)(GModule, uint64_t*, uint64_t*), const char* key,
                           int show_perc, int sp) {
  int isp = 0;
...
static void pmeta_i32_data(GJSON* json, GHolder* h, void (*cb)(GModule, uint32_t*, uint32_t*), const char* key,
                           int show_perc, int sp) {
  int isp = 0;
...
static void pmeta_data_unique(GJSON* json, int ht_size, int sp) {
  int isp = 0;
...
/* Write to a buffer the hits meta data object. */
static void pmeta_data_hits(GJSON* json, GHolder* h, int sp) {
  int isp = 0;
...
static void pmeta_data_visitors(GJSON* json, GHolder* h, int sp) {
  int isp = 0;
...
static void pmeta_data_bw(GJSON* json, GHolder* h, int sp) {
  int isp = 0;
  if (!conf.bandwidth)
    return;
 ...
 * object. */
static void pmeta_data_avgts(GJSON* json, GHolder* h, int sp) {
  int isp = 0;
...
static void pmeta_data_cumts(GJSON* json, GHolder* h, int sp) {
  int isp = 0;
...
static void pmeta_data_maxts(GJSON* json, GHolder* h, int sp) {
  int isp = 0;
  if (!conf.serve_usecs)
    return;
...
static void print_meta_data(GJSON* json, GHolder* h, int sp) {
  int isp = 0, iisp = 0;
  /* use tabs to prettify output */
  if (conf.json_pretty_print)
    isp = sp + 1, iisp = sp + 2;
...
static void print_json_block(GJSON* json, GMetrics* nmetrics, int sp) {
  /* print hits */
...
static void process_host_agents(GJSON* json, GHolderItem* item, int iisp) {
  GAgents* agents = NULL;
  int i, n = 0, iiisp = 0;
...
static void print_json_host_geo(GJSON* json, GSubList* sl, int iisp) {
  GSubItem* iter;
  int i;
...
static void print_data_metrics(GJSON* json, GHolder* h, GPercTotals totals, int sp, const struct GPanel_* panel) {
  GMetrics* nmetrics;
...
static void print_json_data(GJSON* json, GHolder* h, GPercTotals totals, const struct GPanel_* panel) {
  int sp = 0;
  /* use tabs to prettify output */
  if (conf.json_pretty_print)
    sp = 1;
...
static void print_json_summary(GJSON* json, GHolder* holder) {
  int sp = 0, isp = 0;
