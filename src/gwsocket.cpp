...
GWSReader* new_gwsreader(void) {
  GWSReader* reader = cppalloc<GWSReader>();
...
GWSWriter* new_gwswriter(void) {
  GWSWriter* writer = cppalloc<GWSWriter>();
...
static int write_holder(int fd, const char* buf, int len) {
  int i, ret = 0;
...
static void clear_fifo_packet(GWSReader* gwserver) {
...
int broadcast_holder(int fd, const char* buf, int len) {
  ...
  p = cppalloc<char>(sizeof(uint32_t) * 3);
...
int read_fifo(GWSReader* gwsreader, void (*f)(int)) {
  ...
  struct pollfd fds[] = {
      {.fd = gwsreader->self_pipe[0], .events = POLLIN, .revents = 0},
      {.fd = gwsreader->fd, .events = POLLIN, .revents = 0},
  };
...
static int onopen(WSPipeOut* pipeout, WSClient* client) {
  uint32_t hsize = sizeof(uint32_t) * 3;
  char* hdr = cppalloc<char>(hsize);
  char* ptr = hdr;
...
static void* start_server(void* ptr_data) {
