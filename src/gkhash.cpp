...
static GKHashStorage* new_gkhstorage(void) {
  GKHashStorage* storage = cppalloc<GKHashStorage>();
  return storage;
}
...
static GKHashModule* new_gkhmodule(uint32_t size) {
  GKHashModule* storage = cppalloc<GKHashModule>(size);
  return storage;
}
...
static GKHashGlobal* new_gkhglobal(void) {
  GKHashGlobal* storage = cppalloc<GKHashGlobal>();
  return storage;
}
...
static GKHashDB* new_gkhdb(void) {
  GKHashDB* storage = cppalloc<GKHashDB>();
  return storage;
}
...
char* get_mtr_type_str(GSMetricType type) {
  GEnum enum_metric_types[] = {
      {"II32", MTRC_TYPE_II32}, {"IS32", MTRC_TYPE_IS32}, {"IU64", MTRC_TYPE_IU64}, {"SI32", MTRC_TYPE_SI32},
      {"SI08", MTRC_TYPE_SI08}, {"II08", MTRC_TYPE_II08}, {"SS32", MTRC_TYPE_SS32}, {"IGSL", MTRC_TYPE_IGSL},
      {"SU64", MTRC_TYPE_SU64}, {"IGKH", MTRC_TYPE_IGKH}, {"U648", MTRC_TYPE_U648}, {"IGLP", MTRC_TYPE_IGLP},
  };
...
static void des_si32_free(void* h, uint8_t free_data) {
  khint_t k;
  khash_t(si32)* hash = (khash_t(si32)*)h;
  if (!hash)
    return;
...
static void des_si08_free(void* h, uint8_t free_data) {
  khint_t k;
  khash_t(si08)* hash = (khash_t(si08)*)h;
  if (!hash)
    return;
...
static void del_ss32_free(void* h, uint8_t free_data) {
  khint_t k;
  khash_t(ss32)* hash = (khash_t(ss32)*)h;
  if (!hash)
    return;
...
static void des_is32_free(void* h, uint8_t free_data) {
  khint_t k;
  khash_t(is32)* hash = (khash_t(is32)*)h;
  if (!hash)
    return;
...
static void des_ii32(void* h, [[maybe_unused]] uint8_t free_data) {
  khash_t(ii32)* hash = (khash_t(ii32)*)h;
  if (!hash)
    return;
...
static void del_ii32(void* h, [[maybe_unused]] uint8_t free_data) {
  khint_t k;
  khash_t(ii32)* hash = (khash_t(ii32)*)h;
  if (!hash)
    return;
...
/* Destroys the hash structure */
static void des_ii08(void* h, [[maybe_unused]] uint8_t free_data) {
  khash_t(ii08)* hash = (khash_t(ii08)*)h;
  if (!hash)
    return;
...
/* Deletes all entries from the hash table */
static void del_ii08(void* h, [[maybe_unused]] uint8_t free_data) {
  khint_t k;
  khash_t(ii08)* hash = (khash_t(ii08)*)h;
  if (!hash)
    return;
...
/* Destroys the hash structure */
static void des_u648(void* h, [[maybe_unused]] uint8_t free_data) {
  khash_t(u648)* hash = (khash_t(u648)*)h;
  if (!hash)
    return;
...
/* Destroys the hash structure */
static void des_iglp(void* h, [[maybe_unused]] uint8_t free_data) {
  khash_t(iglp)* hash = (khash_t(iglp)*)h;
  if (!hash)
    return;
...
/* Deletes all entries from the hash table */
static void del_u648(void* h, [[maybe_unused]] uint8_t free_data) {
  khint_t k;
  khash_t(u648)* hash = (khash_t(u648)*)h;
  if (!hash)
    return;
...
static void des_igsl_free(void* h, uint8_t free_data) {
  khash_t(igsl)* hash = (khash_t(igsl)*)h;
  khint_t k;
  GSLList* list = NULL;
  if (!hash)
    return;
...
static void des_su64_free(void* h, uint8_t free_data) {
  khash_t(su64)* hash = (khash_t(su64)*)h;
  khint_t k;
  if (!hash)
    return;
...
/* Destroys the hash structure */
static void des_iu64(void* h, [[maybe_unused]] uint8_t free_data) {
  khash_t(iu64)* hash = (khash_t(iu64)*)h;
  if (!hash)
    return;
...
/* Deletes all entries from the hash table */
static void del_iu64(void* h, [[maybe_unused]] uint8_t free_data) {
  khint_t k;
  khash_t(iu64)* hash = (khash_t(iu64)*)h;
  if (!hash)
    return;
...
const GKHashMetric app_metrics[] = {
    ...
    {{.dbm = MTRC_SEQS}, MTRC_TYPE_SI32, new_si32_ht, des_si32_free, del_si32_free, 1, NULL, "SI32_SEQS.db"},
    {{.dbm = MTRC_CNT_OVERALL},
     MTRC_TYPE_SI32,
     new_si32_ht,
     des_si32_free,
     del_si32_free,
     ...
     NULL,
     "SI32_CNT_OVERALL.db"},
    {{.dbm = MTRC_HOSTNAMES}, MTRC_TYPE_SS32, new_ss32_ht, des_ss32_free, del_ss32_free, 1, NULL, NULL},
    {{.dbm = MTRC_LAST_PARSE}, MTRC_TYPE_IGLP, new_iglp_ht, des_iglp, NULL, 1, NULL, "IGLP_LAST_PARSE.db"},
    {{.dbm = MTRC_JSON_LOGFMT}, MTRC_TYPE_SS32, new_ss32_ht, des_ss32_free, del_ss32_free, 1, NULL, NULL},
    {{.dbm = MTRC_METH_PROTO},
     MTRC_TYPE_SI08,
     new_si08_ht,
     des_si08_free,
     ...
     NULL,
     "SI08_METH_PROTO.db"},
    {{.dbm = MTRC_DB_PROPS}, MTRC_TYPE_SI32, new_si32_ht, des_si32_free, del_si32_free, 1, NULL, "SI32_DB_PROPS.db"},
};

/* Per module - These metrics are not dated */
const GKHashMetric global_metrics[] = {
    {{.storem = MTRC_UNIQUE_KEYS},
     MTRC_TYPE_SI32,
     new_si32_ht,
     des_si32_free,
     del_si32_free,
     ...
     NULL,
     "SI32_UNIQUE_KEYS.db"},
    {{.storem = MTRC_AGENT_KEYS}, MTRC_TYPE_II32, new_ii32_ht, des_ii32, del_ii32, 0, NULL, "II32_AGENT_KEYS.db"},
    {{.storem = MTRC_AGENT_VALS},
     MTRC_TYPE_IS32,
     new_is32_ht,
     des_is32_free,
     del_is32_free,
     ...
     NULL,
     "IS32_AGENT_VALS.db"},
    {{.storem = MTRC_CNT_VALID}, MTRC_TYPE_II32, new_ii32_ht, des_ii32, del_ii32, 1, NULL, "II32_CNT_VALID.db"},
    {{.storem = MTRC_CNT_BW}, MTRC_TYPE_IU64, new_iu64_ht, des_iu64, del_iu64, 1, NULL, "IU64_CNT_BW.db"},
};

/* Per module & per date */
GKHashMetric module_metrics[] = {
    {{.storem = MTRC_KEYMAP}, MTRC_TYPE_II32, new_ii32_ht, des_ii32, del_ii32, 1, NULL, NULL},
    {{.storem = MTRC_ROOTMAP}, MTRC_TYPE_IS32, new_is32_ht, des_is32_free, del_is32_free, 1, NULL, NULL},
    {{.storem = MTRC_DATAMAP}, MTRC_TYPE_IS32, new_is32_ht, des_is32_free, del_is32_free, 1, NULL, NULL},
    {{.storem = MTRC_UNIQMAP}, MTRC_TYPE_U648, new_u648_ht, des_u648, del_u648, 1, NULL, NULL},
    {{.storem = MTRC_ROOT}, MTRC_TYPE_II32, new_ii32_ht, des_ii32, del_ii32, 1, NULL, NULL},
    {{.storem = MTRC_HITS}, MTRC_TYPE_II32, new_ii32_ht, des_ii32, del_ii32, 1, NULL, NULL},
    {{.storem = MTRC_VISITORS}, MTRC_TYPE_II32, new_ii32_ht, des_ii32, del_ii32, 1, NULL, NULL},
    {{.storem = MTRC_BW}, MTRC_TYPE_IU64, new_iu64_ht, des_iu64, del_iu64, 1, NULL, NULL},
    {{.storem = MTRC_CUMTS}, MTRC_TYPE_IU64, new_iu64_ht, des_iu64, del_iu64, 1, NULL, NULL},
    {{.storem = MTRC_MAXTS}, MTRC_TYPE_IU64, new_iu64_ht, des_iu64, del_iu64, 1, NULL, NULL},
    {{.storem = MTRC_METHODS}, MTRC_TYPE_II08, new_ii08_ht, des_ii08, del_ii08, 0, NULL, NULL},
    {{.storem = MTRC_PROTOCOLS}, MTRC_TYPE_II08, new_ii08_ht, des_ii08, del_ii08, 0, NULL, NULL},
    {{.storem = MTRC_AGENTS}, MTRC_TYPE_IGSL, new_igsl_ht, des_igsl_free, del_igsl_free, 1, NULL, NULL},
    {{.storem = MTRC_METADATA}, MTRC_TYPE_SU64, new_su64_ht, des_su64_free, del_su64_free, 1, NULL, NULL},
};
...
static void free_app_metrics(GKHashDB* storage) {
  int i, n = 0;
  GKHashMetric mtrc;
...
static void free_global_metrics(GKHashGlobal* ghash) {
  int i, n = 0;
  GKHashMetric mtrc;
...
/* Destroys malloc'd mdule metrics */
static void free_module_metrics(GKHashModule* mhash, GModule module, uint8_t free_data) {
  int i, n = 0;
  GKHashMetric mtrc;
...
static void del_module_metrics(GKHashModule* mhash, GModule module, uint8_t free_data) {
  int i, n = 0;
  GKHashMetric mtrc;
...
static void free_stores(GKHashStorage* store) {
  int module;
  size_t idx = 0;
  ...
  FOREACH_MODULE(idx, module_list) {
    ...
    free_module_metrics(store->mhash, static_cast<GModule>(module), 1);
  }
...
void* get_db_instance(uint32_t key) {
  ...
  khash_t(igdb)* hash = (khash_t(igdb)*)ht_db;
 ...
 *
 * On success, a pointer to that store is returned. */
void* get_hdb(GKDB* db, GAMetric mtrc) { return db->hdb->metrics[mtrc].hash; }

/* Given a hash and a key (date), get the relevant store
 *
 * On error or not found, NULL is returned.
 * On success, a pointer to that store is returned. */
static void* get_store(khash_t(igkh) * hash, uint32_t key) {
...
static void* get_hash_from_store(GKHashStorage* store, int module, GSMetric metric) {
  int mtrc = 0, cnt = 0;
  if (!store)
    return NULL;
...
void* get_hash(int module, uint64_t key, GSMetric metric) {
  GKHashStorage* store = NULL;
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(igkh)* hash = (khash_t(igkh)*)get_hdb(db, MTRC_DATES);

  if ((store = (GKHashStorage*)get_store(hash, key)) == NULL)
    return NULL;
 ...
 *
 * On success, a pointer to that hash table is returned. */
static void* get_hash_from_cache(GModule module, GSMetric metric) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
...
Logs* get_db_logs(uint32_t instance) {
  GKDB* db = (GKDB*)get_db_instance(instance);
  return db->logs;
}
 ...
 *
 * On success, a pointer to that hash structure is returned. */
static GKHashGlobal* init_gkhashglobal(void) {
...
static GKHashModule* init_gkhashmodule(void) {
  ...
  FOREACH_MODULE(idx, module_list) {
    ...
    storage[module].module = static_cast<GModule>(module);
    init_tables(static_cast<GModule>(module), storage);
  }
...
static int ins_igkh(khash_t(igkh) * hash, uint32_t key) {
  GKHashStorage* store = NULL;
  khint_t k;
  int ret;
...
static uint8_t ins_si08_ai(khash_t(si08) * hash, const char* key) {
 ...
 * On error, 0 is returned.
 * On success or if the key exists, the value is returned */
static uint32_t ins_si32_inc(khash_t(si32) * hash, const char* key, uint32_t (*cb)(khash_t(si32) *, const char*),
                             khash_t(si32) * seqs, const char* seqk) {
  khint_t k;
  int ret;
  uint32_t value = 0;
 ...
 * On error, 0 is returned.
 * On success or if the key exists, the value is returned */
static uint32_t ins_ii32_inc(khash_t(ii32) * hash, uint32_t key, uint32_t (*cb)(khash_t(si32) *, const char*),
                             khash_t(si32) * seqs, const char* seqk) {
  khint_t k;
  int ret;
  uint32_t value = 0;
 ...
 * On success 0 is returned */
static int ins_ss32(khash_t(ss32) * hash, const char* key, const char* value) {
  khint_t k;
  int ret;
  char* dupkey = NULL;
...
static uint32_t inc_ii32(khash_t(ii32) * hash, uint32_t key, uint32_t inc) {
  khint_t k;
  int ret;
  uint32_t value = inc;
 ...
 * On error, -1 is returned.
 * On success 0 is returned */
static int inc_su64(khash_t(su64) * hash, const char* key, uint64_t inc) {
  khint_t k;
  int ret;
  uint64_t value = inc;
  char* dupkey = NULL;
 ...
 * On error, -1 is returned.
 * On success 0 is returned */
static int inc_iu64(khash_t(iu64) * hash, uint32_t key, uint64_t inc) {
  khint_t k;
  int ret;
  uint64_t value = inc;
...
static uint32_t inc_si32(khash_t(si32) * hash, const char* key, uint32_t inc) {
  khint_t k;
  int ret;
  uint32_t value = inc;
...
static uint32_t ins_ii32_ai(khash_t(ii32) * hash, uint32_t key) {
  int size = 0, value = 0;
  int ret;
  khint_t k;
...
static int find_int_key_in_list(void* data, void* needle) { return (*(uint32_t*)data) == (*(uint32_t*)needle) ? 1 : 0; }
...
static uint32_t get_si32(khash_t(si32) * hash, const char* key) {
  khint_t k;
...
static uint8_t get_si08(khash_t(si08) * hash, const char* key) {
  khint_t k;
 ...
 * On error, 0 is returned.
 * On success the uint8_t value for the given key is returned */
static uint8_t get_ii08(khash_t(ii08) * hash, uint32_t key) {
  khint_t k;
...
static char* get_is32(khash_t(is32) * hash, uint32_t key) {
  khint_t k;
  char* value = NULL;
 ...
 * On error, -1 is returned.
 * On success the uint32_t value for the given key is returned */
static uint32_t get_ii32(khash_t(ii32) * hash, uint32_t key) {
  khint_t k;
  uint32_t value = 0;
...
static uint64_t get_iu64(khash_t(iu64) * hash, uint32_t key) {
  khint_t k;
  uint64_t value = 0;
 ...
 * On error, or if key is not found, 0 is returned.
 * On success the uint64_t value for the given key is returned */
static uint64_t get_su64(khash_t(su64) * hash, const char* key) {
  khint_t k;
  uint64_t val = 0;
...
static GLastParse get_iglp(khash_t(iglp) * hash, uint64_t key) {
  khint_t k;
  GLastParse lp;
...
GSLList* ht_get_keymap_list_from_key(GModule module, uint32_t key) {
  ...
  khash_t(igkh)* dates = (khash_t(igkh)*)get_hdb(db, MTRC_DATES);
  ...
  for (k = kh_begin(dates); k != kh_end(dates); ++k) {
    if (!kh_exist(dates, k))
      continue;
    if (!(hash = (khash_t(ii32)*)get_hash(module, kh_key(dates, k), MTRC_KEYMAP)))
      continue;
...
static void get_ii32_min_max(khash_t(ii32) * hash, uint32_t* min, uint32_t* max) {
  khint_t k;
  uint32_t curvalue = 0;
  int i;
 ...
 * If the hash structure is empty, no values are set.
 * On success the minimum and maximum values are set. */
static void get_iu64_min_max(khash_t(iu64) * hash, uint64_t* min, uint64_t* max) {
  khint_t k;
  uint64_t curvalue = 0;
  int i;
...
uint32_t ht_get_excluded_ips(void) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(si32)* hash = (khash_t(si32)*)get_hdb(db, MTRC_CNT_OVERALL);
...
uint32_t ht_sum_valid(void) {
  ...
  HT_SUM_VAL(dates, k, {
    if ((hash = (khash_t(ii32)*)get_hash(-1, k, MTRC_CNT_VALID)))
      sum += get_ii32(hash, 1);
  });
...
uint8_t ht_insert_meth_proto(const char* key) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(si08)* hash = (khash_t(si08)*)get_hdb(db, MTRC_METH_PROTO);
  uint8_t val = 0;
...
int ht_insert_last_parse(uint64_t key, GLastParse lp) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(iglp)* hash = (khash_t(iglp)*)get_hdb(db, MTRC_LAST_PARSE);
...
uint32_t ht_inc_cnt_valid(uint32_t date, uint32_t inc) {
  khash_t(ii32)* hash = (khash_t(ii32)*)get_hash(-1, date, MTRC_CNT_VALID);
...
int ht_inc_cnt_bw(uint32_t date, uint64_t inc) {
  khash_t(iu64)* hash = (khash_t(iu64)*)get_hash(-1, date, MTRC_CNT_BW);
...
static uint32_t ht_ins_seq(khash_t(si32) * hash, const char* key) {
  if (!hash)
    return 0;
 ...
 */
static uint64_t u64encode(uint32_t x, uint32_t y) {
...
uint32_t ht_insert_unique_key(uint32_t date, const char* key) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(si32)* seqs = (khash_t(si32)*)get_hdb(db, MTRC_SEQS);
  khash_t(si32)* hash = (khash_t(si32)*)get_hash(-1, date, MTRC_UNIQUE_KEYS);
  uint32_t val = 0;
  char* dupkey = NULL;
...
uint32_t ht_insert_agent_key(uint32_t date, uint32_t key) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(si32)* seqs = (khash_t(si32)*)get_hdb(db, MTRC_SEQS);
  khash_t(ii32)* hash = (khash_t(ii32)*)get_hash(-1, date, MTRC_AGENT_KEYS);
  uint32_t val = 0;
...
int ht_insert_agent_value(uint32_t date, uint32_t key, char* value) {
  khash_t(is32)* hash = (khash_t(is32)*)get_hash(-1, date, MTRC_AGENT_VALS);
  char* dupval = NULL;
...
uint32_t ht_insert_keymap(GModule module, uint32_t date, uint32_t key, uint32_t* ckey) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(si32)* seqs = (khash_t(si32)*)get_hdb(db, MTRC_SEQS);
  khash_t(ii32)* hash = (khash_t(ii32)*)get_hash(module, date, MTRC_KEYMAP);
  khash_t(ii32)* cache = (khash_t(ii32)*)get_hash_from_cache(module, MTRC_KEYMAP);
...
int ht_insert_uniqmap(GModule module, uint32_t date, uint32_t key, uint32_t value) {
  khash_t(u648)* hash = (khash_t(u648)*)get_hash(module, date, MTRC_UNIQMAP);
  uint64_t k = 0;
...
int ht_insert_datamap(GModule module, uint32_t date, uint32_t key, const char* value, uint32_t ckey) {
  khash_t(is32)* hash = (khash_t(is32)*)get_hash(module, date, MTRC_DATAMAP);
  khash_t(is32)* cache = (khash_t(is32)*)get_hash_from_cache(module, MTRC_DATAMAP);
  char* dupval = NULL;
  int ret = 0;
...
int ht_insert_rootmap(GModule module, uint32_t date, uint32_t key, const char* value, uint32_t ckey) {
  khash_t(is32)* hash = (khash_t(is32)*)get_hash(module, date, MTRC_ROOTMAP);
  khash_t(is32)* cache = (khash_t(is32)*)get_hash_from_cache(module, MTRC_ROOTMAP);
  char* dupval = NULL;
  int ret = 0;
...
int ht_insert_root(GModule module, uint32_t date, uint32_t key, uint32_t value, uint32_t dkey, uint32_t rkey) {
  khash_t(ii32)* hash = (khash_t(ii32)*)get_hash(module, date, MTRC_ROOT);
  khash_t(ii32)* cache = (khash_t(ii32)*)get_hash_from_cache(module, MTRC_ROOT);
...
uint32_t ht_insert_hits(GModule module, uint32_t date, uint32_t key, uint32_t inc, uint32_t ckey) {
  khash_t(ii32)* hash = (khash_t(ii32)*)get_hash(module, date, MTRC_HITS);
  khash_t(ii32)* cache = (khash_t(ii32)*)get_hash_from_cache(module, MTRC_HITS);
...
uint32_t ht_insert_visitor(GModule module, uint32_t date, uint32_t key, uint32_t inc, uint32_t ckey) {
  khash_t(ii32)* hash = (khash_t(ii32)*)get_hash(module, date, MTRC_VISITORS);
  khash_t(ii32)* cache = (khash_t(ii32)*)get_hash_from_cache(module, MTRC_VISITORS);
...
int ht_insert_bw(GModule module, uint32_t date, uint32_t key, uint64_t inc, uint32_t ckey) {
  khash_t(iu64)* hash = (khash_t(iu64)*)get_hash(module, date, MTRC_BW);
  khash_t(iu64)* cache = (khash_t(iu64)*)get_hash_from_cache(module, MTRC_BW);
...
int ht_insert_cumts(GModule module, uint32_t date, uint32_t key, uint64_t inc, uint32_t ckey) {
  khash_t(iu64)* hash = (khash_t(iu64)*)get_hash(module, date, MTRC_CUMTS);
  khash_t(iu64)* cache = (khash_t(iu64)*)get_hash_from_cache(module, MTRC_CUMTS);
...
int ht_insert_maxts(GModule module, uint32_t date, uint32_t key, uint64_t value, uint32_t ckey) {
  khash_t(iu64)* hash = (khash_t(iu64)*)get_hash(module, date, MTRC_MAXTS);
  khash_t(iu64)* cache = (khash_t(iu64)*)get_hash_from_cache(module, MTRC_MAXTS);
...
int ht_insert_method(GModule module, uint32_t date, uint32_t key, const char* value, uint32_t ckey) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(ii08)* hash = (khash_t(ii08)*)get_hash(module, date, MTRC_METHODS);
  khash_t(ii08)* cache = (khash_t(ii08)*)get_hash_from_cache(module, MTRC_METHODS);
  khash_t(si08)* mtpr = (khash_t(si08)*)get_hdb(db, MTRC_METH_PROTO);
  int ret = 0;
  uint8_t val = 0;
...
int ht_insert_protocol(GModule module, uint32_t date, uint32_t key, const char* value, uint32_t ckey) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(ii08)* hash = (khash_t(ii08)*)get_hash(module, date, MTRC_PROTOCOLS);
  khash_t(ii08)* cache = (khash_t(ii08)*)get_hash_from_cache(module, MTRC_PROTOCOLS);
  khash_t(si08)* mtpr = (khash_t(si08)*)get_hdb(db, MTRC_METH_PROTO);
  int ret = 0;
  uint8_t val = 0;
...
int ht_insert_agent(GModule module, uint32_t date, uint32_t key, uint32_t value) {
  khash_t(igsl)* hash = (khash_t(igsl)*)get_hash(module, date, MTRC_AGENTS);
...
int ht_insert_meta_data(GModule module, uint32_t date, const char* key, uint64_t value) {
  khash_t(su64)* hash = (khash_t(su64)*)get_hash(module, date, MTRC_METADATA);
...
int ht_insert_hostname(const char* ip, const char* host) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(ss32)* hash = (khash_t(ss32)*)get_hdb(db, MTRC_HOSTNAMES);
...
int ht_insert_json_logfmt([[maybe_unused]] void* userdata, char* key, char* spec) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(ss32)* hash = (khash_t(ss32)*)get_hdb(db, MTRC_JSON_LOGFMT);
  khint_t k;
  int ret;
  char* dupkey = NULL;
...
char* ht_get_root(GModule module, uint32_t key) {
  int root_key = 0;
  khash_t(ii32)* hashroot = (khash_t(ii32)*)get_hash_from_cache(module, MTRC_ROOT);
  khash_t(is32)* hashrootmap = (khash_t(is32)*)get_hash_from_cache(module, MTRC_ROOTMAP);
...
char* ht_get_host_agent_val(uint32_t key) {
  ...
  HT_FIRST_VAL(dates, k, {
    if ((hash = (khash_t(is32)*)get_hash(-1, k, MTRC_AGENT_VALS)))
...
GSLList* ht_get_host_agent_list(GModule module, uint32_t key) {
  ...
  for (k = kh_begin(dates); k != kh_end(dates); ++k) {
    if (!kh_exist(dates, k))
      continue;
    if (!(hash = (khash_t(igsl)*)get_hash(module, kh_key(dates, k), MTRC_AGENTS)))
      continue;
    if ((kv = kh_get(igsl, hash, key)) == kh_end(hash))
      continue;

    list = kh_val(hash, kv);
    GSLIST_FOREACH(list, data, { res = list_insert_prepend(res, i322ptr((*(uint32_t*)data))); });
  }
...
uint32_t ht_get_keymap(GModule module, const char* key) {
  khash_t(si32)* cache = (khash_t(si32)*)get_hash_from_cache(module, MTRC_KEYMAP);
...
uint64_t ht_get_meta_data(GModule module, const char* key) {
  ...
  HT_SUM_VAL(dates, k, {
    if ((hash = (khash_t(su64)*)get_hash(module, k, MTRC_METADATA)))
...
int invalidate_date(int date) {
  ...
  FOREACH_MODULE(idx, module_list) {
    module = module_list[idx];
    del_module_metrics(db->cache, (GModule)module, 0);
  }
...
static uint32_t ins_cache_map(GModule module, GSMetric metric, uint32_t key) {
  khash_t(ii32)* cache = (khash_t(ii32)*)get_hash_from_cache(module, metric);
...
static int ins_cache_ii08(GKHashStorage* store, GModule module, GSMetric metric, uint32_t key, uint32_t ckey) {
  khash_t(ii08)* hash = (khash_t(ii08)*)get_hash_from_store(store, module, metric);
  khash_t(ii08)* cache = (khash_t(ii08)*)get_hash_from_cache(module, metric);
  khint_t k;
...
static int ins_cache_is32(GKHashStorage* store, GModule module, GSMetric metric, uint32_t key, uint32_t ckey) {
  khash_t(is32)* hash = (khash_t(is32)*)get_hash_from_store(store, module, metric);
  khash_t(is32)* cache = (khash_t(is32)*)get_hash_from_cache(module, metric);
  khint_t k;
...
static int inc_cache_ii32(GKHashStorage* store, GModule module, GSMetric metric, uint32_t key, uint32_t ckey) {
  khash_t(ii32)* hash = (khash_t(ii32)*)get_hash_from_store(store, module, metric);
  khash_t(ii32)* cache = (khash_t(ii32)*)get_hash_from_cache(module, metric);
  khint_t k;
...
static int max_cache_iu64(GKHashStorage* store, GModule module, GSMetric metric, uint32_t key, uint32_t ckey) {
  khash_t(iu64)* hash = (khash_t(iu64)*)get_hash_from_store(store, module, metric);
  khash_t(iu64)* cache = (khash_t(iu64)*)get_hash_from_cache(module, metric);
  khint_t k;
...
static int inc_cache_iu64(GKHashStorage* store, GModule module, GSMetric metric, uint32_t key, uint32_t ckey) {
  khash_t(iu64)* hash = (khash_t(iu64)*)get_hash_from_store(store, module, metric);
  khash_t(iu64)* cache = (khash_t(iu64)*)get_hash_from_cache(module, metric);
  khint_t k;
...
static int ins_raw_num_data(GModule module, uint32_t date) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(igkh)* hash = (khash_t(igkh)*)get_hdb(db, MTRC_DATES);
  GKHashStorage* store = (GKHashStorage*)get_store(hash, date);
  khiter_t k, kr;
  uint32_t ckey = 0, rkey = 0, nrkey = 0;
  char* val = NULL;

  khash_t(ii32)* kmap = (khash_t(ii32)*)get_hash_from_store(store, module, MTRC_KEYMAP);
  khash_t(ii32)* root = (khash_t(ii32)*)get_hash_from_store(store, module, MTRC_ROOT);
  khash_t(is32)* rmap = (khash_t(is32)*)get_hash_from_store(store, module, MTRC_ROOTMAP);
  khash_t(ii32)* cache = (khash_t(ii32)*)get_hash_from_cache(module, MTRC_ROOT);
...
static int set_raw_num_data_date(GModule module) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(igkh)* hash = (khash_t(igkh)*)get_hdb(db, MTRC_DATES);
  khiter_t k;
...
static GRawData* init_new_raw_data(GModule module, uint32_t ht_size) {
  GRawData* raw_data;
...
static GRawData* get_u32_raw_data(GModule module) {
  khash_t(ii32)* hash = (khash_t(ii32)*)get_hash_from_cache(module, MTRC_HITS);
  GRawData* raw_data;
  khiter_t key;
  uint32_t ht_size = 0;
...
static GRawData* get_str_raw_data(GModule module) {
  khash_t(is32)* hash = (khash_t(is32)*)get_hash_from_cache(module, MTRC_DATAMAP);
  GRawData* raw_data;
  khiter_t key;
  uint32_t ht_size = 0;
...
/* Destroys the hash structure */
static void des_igkh(void* h) {
  khint_t k;
  khash_t(igkh)* hash = (khash_t(igkh)*)h;
...
static void free_cache(GKHashModule* cache) {
  int module;
  size_t idx = 0;

  FOREACH_MODULE(idx, module_list) {
    module = module_list[idx];
    free_module_metrics(cache, (GModule)module, 0);
  }
  free(cache);
}

static void free_igdb(khash_t(igdb) * hash, khint_t k) {
  GKDB* db = NULL;
  db = kh_val(hash, k);
...
/* Destroys the hash structure */
static void des_igdb(void* h) {
  khint_t k;
  khash_t(igdb)* hash = (khash_t(igdb)*)h;
