...
GRawData* new_grawdata(void) {
  GRawData* raw_data = cppalloc<GRawData>();
...
GRawDataItem* new_grawdata_item(unsigned int size) {
  GRawDataItem* item = cppalloc<GRawDataItem>(size);
  return item;
}
...
Logs* new_logs(int size) {
  Logs* logs = cppalloc<Logs>();
  logs->glog = cppalloc<GLog>(size);
  logs->size = size;
  logs->idx = 0;
...
int set_glog(Logs* logs, const char* filename) {
  ...
  if (logs->size - 1 < logs->idx) {
    newlen = logs->size + 1;
    if (!(tmp = (GLog*)xrealloc(logs->glog, newlen * sizeof(GLog))))
  ...
  glog = logs->glog;
  glog[logs->idx].errors = cppalloc<char*>(MAX_LOG_ERRORS);
...
static int extract_keyphrase(char* ref, char** keyphrase) {
  ...
  if (!(strstr(ref, "http://www.google.")) && !(strstr(ref, "http://webcache.googleusercontent.com/")) &&
      !(strstr(ref, "http://translate.googleusercontent.com/")) && !(strstr(ref, "https://www.google.")) &&
...
static void contains_usecs(void) {
  if (conf.serve_usecs)
    return;
...
static char* parsed_string(const char* pch, char** str, int move_ptr) {
  char* p;
 ...
 * On success, the malloc'd token is returned. */
static char* parse_string(char** str, const char* delims, int cnt) {
  int idx = 0;
  char *pch = *str, *p = NULL;
  char end;
...
char* extract_by_delim(char** str, const char* end) { return parse_string(&(*str), end, 1); }
...
static int find_alpha_count(char* str) {
  int cnt = 0;
  char* s = str;
  while (*s) {
    if (isspace(*s))
...
static int spec_err(GLogItem* logitem, int code, const char spec, const char* tkn) {
  char* err = NULL;
  const char* fmt = NULL;
  ...
  case ERR_SPEC_TOKN_NUL:
    ...
    err = cppalloc<char>(snprintf(NULL, 0, fmt, spec) + 1);
    sprintf(err, fmt, spec);
    break;
  case ERR_SPEC_TOKN_INV:
    fmt = "Token '%s' doesn't match specifier '%%%c'";
    err = cppalloc<char>(snprintf(NULL, 0, fmt, (tkn ? tkn : "-"), spec) + 1);
  ...
  case ERR_SPEC_LINE_INV:
    ...
    err = cppalloc<char>(snprintf(NULL, 0, fmt, (tkn ? tkn : "-")) + 1);
...
static void set_agent_hash(GLogItem* logitem) {
...
static int parse_specifier(GLogItem* logitem, char** str, const char* p, const char* end) {
  struct tm tm;
  time_t now = time(0);
  ...
  case 'x':
    ...
    if (str_to_time(tkn, tfmt, &tm, 1) != 0 || set_date(&logitem->date, tm) != 0 || set_time(&logitem->time, tm) != 0) {
      spec_err(logitem, ERR_SPEC_TOKN_INV, *p, tkn);
      free(tkn);
      return 1;
    }
...
static char* extract_braces(char** p) {
 ...
 * 0 is returned. */
static int set_xff_host(GLogItem* logitem, char** str, char* skips, int out) {
...
static int find_xff_host(GLogItem* logitem, char** str, char** p) {
 ...
 * 0 is returned. */
static int special_specifier(GLogItem* logitem, char** str, char** p) {
  switch (**p) {
 ...
 * On success, the malloc'd token is assigned to a GLogItem member and
 * 0 is returned. */
static int parse_format(GLogItem* logitem, char* str, char* lfmt) {
  char end[2 + 1] = {0};
  char* p = NULL;
...
static int handle_crawler(const char* agent) {
  int bot = 0;
...
static int is_static(const char* req) { return verify_static_content(req); }
 ...
 * If the request line is not ignored, 0 is returned.
 * If the request line is ignored, 1 is returned. */
static int ignore_static(const char* req) {
...
static int is_404(GLogItem* logitem) {
...
static int ignore_line(GLogItem* logitem) {
...
static char* get_uniq_visitor_key(GLogItem* logitem) {
  char* key = NULL;
  size_t s1, s2, s3;
...
static int is_likely_same_log(GLog* glog, const GLastParse* lp) {
  size_t size = 0;
...
static int should_restore_from_disk(GLog* glog) {
  GLastParse lp;
...
static void process_invalid(GLog* glog, GLogItem* logitem, const char* line) {
  GLastParse lp;
  ::memset(&lp, 0, sizeof(lp));
...
static int read_line(GLog* glog, char* line, int* test, int* cnt, int dry_run) {
  int ret = 0;
...
char* fgetline(FILE* fp) {
  ...
  while (1) {
    ...
    if ((tmp = (char*)realloc(line, linelen + len + 1)) == NULL)
      break;
 ...
 * On success, 0 is returned. */
#ifdef WITH_GETLINE
static int read_lines(FILE* fp, GLog* glog, int dry_run) {
  char* line = NULL;
...
static void persist_last_parse(GLog* glog) {
 ...
 * On error, 1 is returned.
 * On success, 0 is returned. */
static int read_log(GLog* glog, int dry_run) {
  FILE* fp = NULL;
  int piping = 0;
  struct stat fdstat;
...
static void set_log_processing(Logs* logs, GLog* glog) {
  lock_spinner();
...
char** test_format(Logs* logs, int* len) {
  ...
  errors = cppalloc<char*>(glog->log_erridx);
  *len = glog->log_erridx;
  for (i = 0; i < glog->log_erridx; ++i)
