...
// TODO: this here is one of the problems; this list is iterated through for each line.
// This could be done better; but for the time being our purpose is to preserve the
// original meaning, not to affect it. So most changes will be whitespaces and
// other cosmetics.
...
static void set_browser(char*** list, int idx, const char* browser, const char* type) {
  list[idx] = cppalloc<char*>(2); // This will probably be two strings, but let's wait for a bit.
...
void parse_browsers_file(void) {
  ...
  browsers_hash = cppalloc<char**>(ARRAY_SIZE(browsers));
  ...
  conf.user_browsers_hash = cppalloc<char**>(MAX_CUSTOM_BROWSERS);
...
static char* parse_opera(char* token) {
  char* val = cppalloc<char>(snprintf(NULL, 0, "Opera%s", token) + 1);
