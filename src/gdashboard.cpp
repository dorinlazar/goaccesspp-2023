...
void reset_find(void) {
  ...
  find_t.module = static_cast<GModule>(0);
...
GDashData* new_gdata(uint32_t size) {
  GDashData* data = cppalloc<GDashData>(size);
 ...
 * strings. */
static void free_dashboard_data(GDashData item) {
...
static GModule get_find_current_module(GDash* dash, int offset) {
  int module;
  size_t idx = 0;
  ...
  FOREACH_MODULE(idx, module_list) {
    ...
    if (dash->module[module].pos_y == offset)
      return static_cast<GModule>(module);
    /* we went over by one module, set current - 1 */
    if (dash->module[module].pos_y > offset)
      return static_cast<GModule>(module - 1);
  }

  return static_cast<GModule>(0);
}
...
static int get_data_pos_rows(void) { return conf.no_column_names ? DASH_DATA_POS - DASH_COL_ROWS : DASH_DATA_POS; }
...
static int get_xpos(void) { return DASH_INIT_X; }
...
int set_module_from_mouse_event(GScroll* gscroll, GDash* dash, int y) {
  int module = 0;
  ...
  gscroll->current = static_cast<GModule>(module);
  return 0;
}
...
static void set_max_metrics(GDashMeta* meta, GDashData* idata) {
...
static void set_max_hit_len(GDashMeta* meta, GDashData* idata) {
...
static void set_max_hit_perc_len(GDashMeta* meta, GDashData* idata) {
...
/* Set largest hits metric (length of the integer). */
static void set_max_visitors_len(GDashMeta* meta, GDashData* idata) {
...
/* Get the percent integer length. */
static void set_max_visitors_perc_len(GDashMeta* meta, GDashData* idata) {
...
/* Get the percent integer length. */
static void set_max_bw_len(GDashMeta* meta, GDashData* idata) {
...
/* Get the percent integer length. */
static void set_max_avgts_len(GDashMeta* meta, GDashData* idata) {
  int vlen = 0, llen = 0;
...
/* Get the percent integer length. */
static void set_max_cumts_len(GDashMeta* meta, GDashData* idata) {
  int vlen = 0, llen = 0;
...
/* Get the percent integer length. */
static void set_max_maxts_len(GDashMeta* meta, GDashData* idata) {
  int vlen = 0, llen = 0;
...
/* Get the percent integer length. */
static void set_max_method_len(GDashMeta* meta, GDashData* idata) {
  int vlen = 0, llen = 0;
...
/* Get the percent integer length. */
static void set_max_protocol_len(GDashMeta* meta, GDashData* idata) {
  int vlen = 0, llen = 0;
...
/* Get the percent integer length. */
static void set_max_data_len(GDashMeta* meta, GDashData* idata) {
  int vlen = 0, llen = 0;
...
static void render_data_hosts(WINDOW* win, GDashRender render, char* value, int x) {
  char* padded_data;
...
static char* set_visitors_date(const char* value) {
  return get_visitors_date(value, conf.spec_date_time_num_format, conf.spec_date_time_format);
}

static char* get_fixed_fmt_width(int w, char type) {
  char* fmt = cppalloc<char>(snprintf(NULL, 0, "%%%d%c", w, type) + 1);
...
static void render_total_label(WINDOW* win, GDashModule* data, int y, GColors* (*func)(void)) {
  char* s;
  ...
  s = cppalloc<char>(snprintf(NULL, 0, "%s: %d/%d", GEN_TOTAL, total, ht_size) + 1);
...
static void render_bars(GDashModule* data, GDashRender render, int* x) {
...
static void render_method(GDashModule* data, GDashRender render, int* x) {
...
static void render_proto(GDashModule* data, GDashRender render, int* x) {
...
static void render_avgts(GDashModule* data, GDashRender render, int* x) {
...
static void render_cumts(GDashModule* data, GDashRender render, int* x) {
...
static void render_maxts(GDashModule* data, GDashRender render, int* x) {
...
static void render_bw(GDashModule* data, GDashRender render, int* x) {
...
static void render_percent(GDashRender render, GColors* color, float perc, int len, int x) {
  WINDOW* win = render.win;
  char* percent;
...
static void render_hits_percent(GDashModule* data, GDashRender render, int* x) {
...
static void render_visitors_percent(GDashModule* data, GDashRender render, int* x) {
...
static void render_hits(GDashModule* data, GDashRender render, int* x) {
...
static void render_visitors(GDashModule* data, GDashRender render, int* x) {
...
static void render_header(WINDOW* win, GDashModule* data, GModule cur_module, int* y) {
  GColors* (*func)(void);
  char ind;
  char* hd;
  int k, w, h;
  ...
  hd = cppalloc<char>(snprintf(NULL, 0, "%c %d - %s", ind, k, data->head) + 1);
...
static void render_description(WINDOW* win, GDashModule* data, int* y) {
  int w, h;
...
static void render_metrics(GDashModule* data, GDashRender render, int expanded) {
  int x = get_xpos();
...
static void render_data_line(WINDOW* win, GDashModule* data, int* y, int j, GScroll* gscroll) {
  GDashRender render;
  GModule module = data->module;
...
static void print_horizontal_dash(WINDOW* win, int y, int x, int len) {
  mvwprintw(win, y, x, "%.*s", len, "----------------");
}

/* Render left-aligned column label. */
static void lprint_col(WINDOW* win, int y, int* x, int len, const char* str) {
...
static void rprint_col(WINDOW* win, int y, int* x, int len, const char* str) {
  GColors* color = get_color(COLOR_PANEL_COLS);
 ...
 * ###TODO: Have the abilitity to display metrics in specific order */
static void render_cols(WINDOW* win, GDashModule* data, int* y) {
  GModule module = data->module;
  const GOutput* output = output_lookup(module);
  int x = get_xpos();
...
static void render_content(WINDOW* win, GDashModule* data, int* y, int* offset, int* total, GScroll* gscroll) {
  GModule module = data->module;
...
void display_content(WINDOW* win, GDash* dash, GScroll* gscroll) {
  int module;
  int j = 0;
  size_t idx = 0;
  ...
  FOREACH_MODULE(idx, module_list) {
    ...
    /* used module */
    dash->module[module].module = static_cast<GModule>(module);
...
void reset_scroll_offsets(GScroll* gscroll) {
  int module;
  size_t idx = 0;
...
static int regexp_init(regex_t* regex, const char* pattern) {
  int y, x, rc;
...
static void perform_find_dash_scroll(GScroll* gscroll, GModule module) {
  int *scrll, *offset;
...
static int find_next_sub_item(GSubList* sub_list, regex_t* regex) {
  GSubItem* iter;
  int i = 0, rc;
...
int perform_next_find(GHolder* h, GScroll* gscroll) {
  int module;
  GSubList* sub_list;
  regex_t regex;
  ...
  FOREACH_MODULE(idx, module_list) {
    ...
    for (j = find_t.next_parent_idx; j < n; j++, find_t.next_idx++) {
      ...
      /* a match was found (data level) */
        ...
        perform_find_dash_scroll(gscroll, static_cast<GModule>(module));
        goto out;
      }
...
int render_find_dialog(WINDOW* main_win, GScroll* gscroll) {
  int y, x, valid = 1;
...
static void set_dash_metrics(GDash** dash, GMetrics* metrics, GModule module, GPercTotals totals, int is_subitem) {
...
static void add_sub_item_to_dash(GDash** dash, GHolderItem item, GModule module, GPercTotals totals, int* i) {
...
static void add_item_to_dash(GDash** dash, GHolderItem item, GModule module, GPercTotals totals) {
  set_dash_metrics(dash, item.metrics, module, totals, 0);
}

/* Load holder's data into the dashboard structure. */
void load_data_to_dash(GHolder* h, GDash* dash, GModule module, GScroll* gscroll) {
  int alloc_size = 0;
  int i, j;
  GPercTotals totals;
