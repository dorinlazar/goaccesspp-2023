...
static const char* chart2str(GChartType type) {
...
static GHTML* panel_lookup(GModule module) {
...
static void print_html_title(FILE* fp) {
...
static void print_html_body(FILE* fp, const char* now) {
  fprintf(fp,
...
static int get_chartdef_cnt(GChart* chart) {
...
static void print_d3_chart_def_axis(FILE* fp, GChart* chart, size_t cnt, int isp) {
  GChartDef* def = chart->def;
  size_t j = 0;
...
static void print_plot_def(FILE* fp, const GHTMLPlot plot, GChart* chart, int n, int sp) {
  int isp = 0, iisp = 0;
...
static void print_json_data(FILE* fp, GHolder* holder) {
  char* json = NULL;
...
static void print_conn_def(FILE* fp) {
  int sp = 0;
  /* use tabs to prettify output */
  if (conf.json_pretty_print)
    sp += 1;
...
static void print_def_metric(FILE* fp, const GDefMetric def, int sp) {
  int isp = 0;
...
static void print_def_block(FILE* fp, const GDefMetric def, int sp, int last) {
  fpopen_obj(fp, sp);
...
static int count_plot_fp(const GHTML* def) {
  int i = 0;
...
static void print_def_plot(FILE* fp, const GHTML* def, int sp) {
...
static void print_def_metrics(FILE* fp, const GHTML* def, int sp) {
  int isp = 0;
  /* use tabs to prettify output */
  if (conf.json_pretty_print)
    isp = sp + 1;
...
static void print_def_meta(FILE* fp, const char* head, const char* desc, int sp) {
  int isp = 0;
  /* use tabs to prettify output */
  if (conf.json_pretty_print)
    isp = sp + 1;
...
static void print_def_sort(FILE* fp, const GHTML* def, int sp) {
...
static void print_panel_def_meta(FILE* fp, const GHTML* def, int sp) {
...
static void print_json_def(FILE* fp, const GHTML* def) {
  int sp = 0;
  /* use tabs to prettify output */
  if (conf.json_pretty_print)
    sp = 1;
...
static void print_def_summary(FILE* fp, int sp) {
  int isp = 0, iisp = 0;
  /* use tabs to prettify output */
  if (conf.json_pretty_print)
    isp = sp + 1, iisp = sp + 2;
...
static void print_json_i18n_def(FILE* fp) {
  int sp = 0;
  size_t i = 0;
...
/* Output definitions for the given panel. */
static void print_json_def_summary(FILE* fp) {
  int sp = 0;
...
static void print_json_defs(FILE* fp) {
  const GHTML* def;
  size_t idx = 0;
