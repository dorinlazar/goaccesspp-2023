...
static int in_ignore_cmd_opts(const char* val) {
  size_t i;
...
void set_default_static_files(void) {
  size_t i;
  const char* exts[] = {
      ".css",  ".js ",  ".jpg",  ".png",  ".gif",   ".ico",  ".jpeg", ".pdf",  ".txt",  ".csv",
      ".mpeg", ".mpg",  ".swf",  ".woff", ".woff2", ".xls",  ".xlsx", ".doc ", ".docx", ".ppt ",
      ".pptx", ".zip",  ".mp3",  ".mp4",  ".exe",   ".iso ", ".gz  ", ".rar ", ".svg ", ".bmp ",
...
static void append_to_argv(int* argc, char*** argv, char* val) {
...
int parse_conf_file(int* argc, char*** argv) {
  ...
  append_to_argv(&nargc, &nargv, xstrdup(*argv[0] ?: "gpaccess"));
  ...
  while (fgets(line, sizeof line, file) != NULL) {
    ...
    opt = cppalloc<char>(snprintf(NULL, 0, "--%s", line) + 1);
    sprintf(opt, "--%s", line);
...
char* get_selected_format_str(size_t idx) {
  char* fmt = NULL;
  switch (idx) {
  case COMBINED: fmt = alloc_string(logs.combined); break;
  case VCOMBINED: fmt = alloc_string(logs.vcombined); break;
  case COMMON: fmt = alloc_string(logs.common); break;
  case VCOMMON: fmt = alloc_string(logs.vcommon); break;
  case W3C: fmt = alloc_string(logs.w3c); break;
  case CLOUDFRONT: fmt = alloc_string(logs.cloudfront); break;
  case CLOUDSTORAGE: fmt = alloc_string(logs.cloudstorage); break;
  case AWSELB: fmt = alloc_string(logs.awselb); break;
  case SQUID: fmt = alloc_string(logs.squid); break;
  case AWSS3: fmt = alloc_string(logs.awss3); break;
  case CADDY: fmt = alloc_string(logs.caddy); break;
  case AWSALB: fmt = alloc_string(logs.awsalb); break;
  }
...
char* get_selected_date_str(size_t idx) {
  ...
  case COMBINED:
  case VCOMBINED:
  case AWSS3: fmt = alloc_string(dates.apache); break;
  case AWSELB:
  case AWSALB:
  case CLOUDFRONT:
  case W3C: fmt = alloc_string(dates.w3c); break;
  case CLOUDSTORAGE: fmt = alloc_string(dates.usec); break;
  case SQUID:
  case CADDY: fmt = alloc_string(dates.sec); break;
  }
...
char* get_selected_time_str(size_t idx) {
  ...
  case VCOMMON:
  case W3C:
  case AWSS3: fmt = alloc_string(times.fmt24); break;
  case CLOUDSTORAGE: fmt = alloc_string(times.usec); break;
  case SQUID:
  case CADDY: fmt = alloc_string(times.sec); break;
  }
 ...
 * format. */
static char* append_spec_date_format(const char* date_format, const char* spec_format) {
  char* s = cppalloc<char>(snprintf(NULL, 0, "%s%s", date_format, spec_format) + 1);
...
int is_json_log_format(const char* fmt) {
  ...
  do {
    t = json_next(&json);
    switch (t) {
    case JSON_ERROR: json_close(&json); return 0;
    default: break;
    }
...
static void dec_json_key(char* key) {
  char* ptr = NULL;
...
int parse_json_string(void* ptr_data, const char* str, int (*cb)(void*, char*, char*)) {
  ...
  do {
    ...
    case JSON_OBJECT:
      ...
      break;
    case JSON_ARRAY_END:
    case JSON_OBJECT_END: dec_json_key(key); break;
    case JSON_TRUE:
      val = xstrdup("true");
