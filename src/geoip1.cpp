...
int is_geoip_resource(void) { return ((geoips && db_cnt) || (legacy_db && geo_location_data)); }
...
static GeoIP* geoip_open_db(const char* db) {
...
static int set_geoip_db_by_type(GeoIP* geoip, GO_GEOIP_DB type) {
...
static int set_conf_by_type(GeoIP* geoip) {
  unsigned char rec = GeoIP_database_edition(geoip);

  switch (rec) {
  case GEOIP_ASNUM_EDITION: conf.has_geoasn = 1; return 0;
  case GEOIP_COUNTRY_EDITION:
  case GEOIP_COUNTRY_EDITION_V6: conf.has_geocountry = 1; return 0;
  case GEOIP_CITY_EDITION_REV0:
  case GEOIP_CITY_EDITION_REV1:
  case GEOIP_CITY_EDITION_REV0_V6:
  case GEOIP_CITY_EDITION_REV1_V6: conf.has_geocountry = conf.has_geocity = 1; return 0;
  }
...
static int set_geoip_db(GO_GEOIP_DB type) {
  int idx = 0;
...
static void set_geoip(const char* db) {
  ...
  new_geoips = static_cast<GeoIP**>(realloc(geoips, sizeof(*geoips) * db_cnt));
  if (new_geoips == NULL)
...
/* Compose a string with the continent name and store it in the given
 * buffer. */
static void geoip_set_asn(const char* name, char* asn) {
  if (name)
...
static GeoIPRecord* get_geoip_record(const char* addr, GTypeIP type_ip) {
 ...
 * on the IP version. */
static void geoip_set_country_by_record(const char* ip, char* location, GTypeIP type_ip) {
  GeoIPRecord* rec = NULL;
...
static int geoip_get_geoid(const char* addr, GTypeIP type_ip) {
  int geoid = 0;
...
static const char* geoip_get_country_by_geoid(const char* addr, GTypeIP type_ip) {
 ...
 * the IP version. */
static void geoip_set_country_by_geoid(const char* ip, char* location, GTypeIP type_ip) {
  const char *country = NULL, *code = NULL, *addr = ip;
  int geoid = 0;
 ...
 * on the IP version. */
static void geoip_set_continent_by_record(const char* ip, char* location, GTypeIP type_ip) {
  GeoIPRecord* rec = NULL;
 ...
 * on the IP version. */
static void geoip_set_continent_by_geoid(const char* ip, char* location, GTypeIP type_ip) {
  const char *continent = NULL, *addr = ip;
  int geoid = 0;
 ...
 * the IP version.  */
static void geoip_set_city_by_record(const char* ip, char* location, GTypeIP type_ip) {
  GeoIPRecord* rec = NULL;
...
static void geoip_get_city(const char* ip, char* location, GTypeIP type_ip) {
  unsigned char rec = 0;
...
int set_geolocation(char* host, char* continent, char* country, char* city, GO_UNUSED char* asn) {
  ...
  if (set_geoip_db(TYPE_COUNTRY) == 0 || set_geoip_db(TYPE_CITY) == 0) {
    geoip_get_country(host, country, static_cast<GTypeIP>(type_ip));
    geoip_get_continent(host, continent, static_cast<GTypeIP>(type_ip));
  }
  if (set_geoip_db(TYPE_CITY) == 0)
    geoip_get_city(host, city, static_cast<GTypeIP>(type_ip));
