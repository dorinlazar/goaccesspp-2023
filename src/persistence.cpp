...
static char* set_db_path(const char* fn) {
  struct stat info;
  ...
  path = cppalloc<char>(snprintf(NULL, 0, "%s/%s", rpath, fn) + 1);
...
static char* build_filename(const char* type, const char* modstr, const char* mtrstr) {
  char* fn = cppalloc<char>(snprintf(NULL, 0, "%s_%s_%s.db", type, modstr, mtrstr) + 1);
...
static char* get_filename(GModule module, GKHashMetric mtrc) {
...
static void close_tpl(tpl_node* tn, const char* fn) {
 ...
 * storage */
static void restore_global_si08(khash_t(si08) * hash, const char* fn) {
  tpl_node* tn;
  char* key = NULL;
  char fmt[] = "A(sv)";
  uint16_t val;
...
static void persist_global_si08(khash_t(si08) * hash, const char* fn) {
  tpl_node* tn;
  khint_t k;
  const char* key = NULL;
  char fmt[] = "A(sv)";
...
/* Given a database filename, restore a string key, uint32_t value back to the
 * storage */
static void restore_global_si32(khash_t(si32) * hash, const char* fn) {
  tpl_node* tn;
  char* key = NULL;
  char fmt[] = "A(su)";
  uint32_t val;
...
/* Given a hash and a filename, persist to disk a string key, uint32_t value */
static void persist_global_si32(khash_t(si32) * hash, const char* fn) {
  tpl_node* tn;
  khint_t k;
  const char* key = NULL;
  char fmt[] = "A(su)";
 ...
 * the storage */
static void restore_global_iglp(khash_t(iglp) * hash, const char* fn) {
  tpl_node* tn;
  uint64_t key;
  GLastParse val;
...
static void persist_global_iglp(khash_t(iglp) * hash, const char* fn) {
  tpl_node* tn;
  khint_t k;
  uint64_t key;
  GLastParse val;
...
static int insert_restored_date(uint32_t date) {
  uint32_t i, len = 0;
 ...
 * the storage */
static int restore_si32(GSMetric metric, const char* path, int module) {
  ...
  while (tpl_unpack(tn, 1) > 0) {
    if ((ret = insert_restored_date(date)) == 2)
      continue;
    if (ret == -1 || !(hash = (khash_t(si32)*)get_hash(module, date, metric)))
      break;
...
/* Given a database filename, restore a string key, uint32_t value back to
 * the storage */
static int migrate_si32_to_ii32(GSMetric metric, const char* path, int module) {
  ...
  while (tpl_unpack(tn, 1) > 0) {
    if ((ret = insert_restored_date(date)) == 2)
      continue;
    if (ret == -1 || !(hash = (khash_t(ii32)*)get_hash(module, date, metric)))
      break;
...
static char* migrate_unique_key(char* key) {
...
/* Given a database filename, restore a string key, uint32_t value back to
 * the storage */
static int migrate_si32_to_ii32_unique_keys(GSMetric metric, const char* path, int module) {
  khash_t(si32)* hash = NULL;
  tpl_node* tn;
  char fmt[] = "A(iA(su))";
  int date = 0, ret = 0;
 ...
 * the storage */
static int migrate_is32_to_ii08(GSMetric metric, const char* path, int module) {
  ...
  while (tpl_unpack(tn, 1) > 0) {
    if ((ret = insert_restored_date(date)) == 2)
      continue;
    if (ret == -1 || !(hash = (khash_t(ii08)*)get_hash(module, date, metric)))
      break;
...
/* Given a database filename, restore a uint32_t key, string value back to
 * the storage */
static int restore_is32(GSMetric metric, const char* path, int module) {
  ...
  while (tpl_unpack(tn, 1) > 0) {
    if ((ret = insert_restored_date(date)) == 2)
      continue;
    if (ret == -1 || !(hash = (khash_t(is32)*)get_hash(module, date, metric)))
      break;
...
static int persist_is32(GSMetric metric, const char* path, int module) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(igkh)* dates = (khash_t(igkh)*)get_hdb(db, MTRC_DATES);
  khash_t(is32)* hash = NULL;
  tpl_node* tn = NULL;
 ...
 * the storage */
static int restore_ii08(GSMetric metric, const char* path, int module) {
  khash_t(ii08)* hash = NULL;
  tpl_node* tn;
  char fmt[] = "A(iA(uv))";
  int date = 0, ret = 0;
...
/* Given a database filename, restore a uint32_t key, uint32_t value back to
 * the storage */
static int restore_ii32(GSMetric metric, const char* path, int module) {
  khash_t(ii32)* hash = NULL;
  tpl_node* tn;
  char fmt[] = "A(iA(uu))";
  int date = 0, ret = 0;
...
static int persist_ii32(GSMetric metric, const char* path, int module) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(igkh)* dates = (khash_t(igkh)*)get_hdb(db, MTRC_DATES);
  khash_t(ii32)* hash = NULL;
  tpl_node* tn = NULL;
...
/* Given a hash and a filename, persist to disk a uint32_t key, uint32_t value */
static int persist_ii08(GSMetric metric, const char* path, int module) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(igkh)* dates = (khash_t(igkh)*)get_hdb(db, MTRC_DATES);
  khash_t(ii08)* hash = NULL;
  tpl_node* tn = NULL;
 ...
 * the storage */
static int restore_u648(GSMetric metric, const char* path, int module) {
  ...
  while (tpl_unpack(tn, 1) > 0) {
    if ((ret = insert_restored_date(date)) == 2)
      continue;
    if (ret == -1 || !(hash = (khash_t(u648)*)get_hash(module, date, metric)))
      break;
...
static int persist_u648(GSMetric metric, const char* path, int module) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(igkh)* dates = (khash_t(igkh)*)get_hdb(db, MTRC_DATES);
  khash_t(u648)* hash = NULL;
  tpl_node* tn = NULL;
 ...
 * the storage */
static int restore_iu64(GSMetric metric, const char* path, int module) {
  ...
  while (tpl_unpack(tn, 1) > 0) {
    if ((ret = insert_restored_date(date)) == 2)
      continue;
    if (ret == -1 || !(hash = (khash_t(iu64)*)get_hash(module, date, metric)))
      break;
...
static int persist_iu64(GSMetric metric, const char* path, int module) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(igkh)* dates = (khash_t(igkh)*)get_hdb(db, MTRC_DATES);
  khash_t(iu64)* hash = NULL;
  tpl_node* tn = NULL;
 ...
 * the storage */
static int restore_su64(GSMetric metric, const char* path, int module) {
  ...
  while (tpl_unpack(tn, 1) > 0) {
    if ((ret = insert_restored_date(date)) == 2)
      continue;
    if (ret == -1 || !(hash = (khash_t(su64)*)get_hash(module, date, metric)))
      break;
...
static int persist_su64(GSMetric metric, const char* path, int module) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(igkh)* dates = (khash_t(igkh)*)get_hdb(db, MTRC_DATES);
  khash_t(su64)* hash = NULL;
  tpl_node* tn = NULL;
 ...
 * storage */
static int restore_igsl(GSMetric metric, const char* path, int module) {
  ...
  while (tpl_unpack(tn, 1) > 0) {
    if ((ret = insert_restored_date(date)) == 2)
      continue;
    if (ret == -1 || !(hash = (khash_t(igsl)*)get_hash(module, date, metric)))
      break;
...
static int persist_igsl(GSMetric metric, const char* path, int module) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(igkh)* dates = (khash_t(igkh)*)get_hdb(db, MTRC_DATES);
  khash_t(igsl)* hash = NULL;
  GSLList* node;
...
static char* check_restore_path(const char* fn) {
...
static void restore_by_type(GKHashMetric mtrc, const char* fn, int module) {
  char* path = NULL;
  ...
  switch (mtrc.type) {
  case MTRC_TYPE_SI32: restore_si32(mtrc.metric.storem, path, module); break;
  case MTRC_TYPE_IS32: restore_is32(mtrc.metric.storem, path, module); break;
  case MTRC_TYPE_II08: restore_ii08(mtrc.metric.storem, path, module); break;
  case MTRC_TYPE_II32: restore_ii32(mtrc.metric.storem, path, module); break;
  case MTRC_TYPE_U648: restore_u648(mtrc.metric.storem, path, module); break;
  case MTRC_TYPE_IU64: restore_iu64(mtrc.metric.storem, path, module); break;
  case MTRC_TYPE_SU64: restore_su64(mtrc.metric.storem, path, module); break;
  case MTRC_TYPE_IGSL: restore_igsl(mtrc.metric.storem, path, module); break;
  default: break;
  }
clean:
  free(path);
...
static void restore_metric_type(GModule module, GKHashMetric mtrc) {
  char* fn = NULL;
...
static int migrate_metric(GModule module, GKHashMetric mtrc) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(si32)* db_props = (khash_t(si32)*)get_hdb(db, MTRC_DB_PROPS);
...
static void persist_dates(void) {
  tpl_node* tn;
  char* path = NULL;
...
static void restore_dates(void) {
  tpl_node* tn;
  char* path = NULL;
  uint32_t date, idx = 0;
  char fmt[] = "A(u)";
  ...
  persisted_dates = cppalloc<uint32_t>(persisted_dates_len);
  while (tpl_unpack(tn, 1) > 0)
...
static void restore_global(void) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(si32)* overall = (khash_t(si32)*)get_hdb(db, MTRC_CNT_OVERALL);
  khash_t(si32)* seqs = (khash_t(si32)*)get_hdb(db, MTRC_SEQS);
  khash_t(iglp)* last_parse = (khash_t(iglp)*)get_hdb(db, MTRC_LAST_PARSE);
  khash_t(si32)* db_props = (khash_t(si32)*)get_hdb(db, MTRC_DB_PROPS);
  khash_t(si08)* meth_proto = (khash_t(si08)*)get_hdb(db, MTRC_METH_PROTO);
...
void restore_data(void) {
  ...
  for (i = 0; i < n; ++i) {
    migrated += migrate_metric((GModule)-1, global_metrics[i]);
  ...
  FOREACH_MODULE(idx, module_list) {
    ...
    for (i = 0; i < n; ++i) {
      migrated += migrate_metric((GModule)module, module_metrics[i]);
      restore_metric_type((GModule)module, module_metrics[i]);
    }
  }
...
static void persist_by_type(GKHashMetric mtrc, const char* fn, int module) {
  char* path = NULL;
  path = set_db_path(fn);

  switch (mtrc.type) {
  case MTRC_TYPE_SI32: persist_si32(mtrc.metric.storem, path, module); break;
  case MTRC_TYPE_IS32: persist_is32(mtrc.metric.storem, path, module); break;
  case MTRC_TYPE_II32: persist_ii32(mtrc.metric.storem, path, module); break;
  case MTRC_TYPE_II08: persist_ii08(mtrc.metric.storem, path, module); break;
  case MTRC_TYPE_U648: persist_u648(mtrc.metric.storem, path, module); break;
  case MTRC_TYPE_IU64: persist_iu64(mtrc.metric.storem, path, module); break;
  case MTRC_TYPE_SU64: persist_su64(mtrc.metric.storem, path, module); break;
  case MTRC_TYPE_IGSL: persist_igsl(mtrc.metric.storem, path, module); break;
  default: break;
  }
  free(path);
}

static void persist_metric_type(GModule module, GKHashMetric mtrc) {
  char* fn = NULL;
  fn = get_filename(module, mtrc);
...
static void persist_global(void) {
  GKDB* db = (GKDB*)get_db_instance(DB_INSTANCE);
  khash_t(si32)* overall = (khash_t(si32)*)get_hdb(db, MTRC_CNT_OVERALL);
  khash_t(si32)* seqs = (khash_t(si32)*)get_hdb(db, MTRC_SEQS);
  khash_t(iglp)* last_parse = (khash_t(iglp)*)get_hdb(db, MTRC_LAST_PARSE);
...
void persist_data(void) {
  ...
  FOREACH_MODULE(idx, module_list) {
    module = module_list[idx];
    for (i = 0; i < n; ++i) {
      persist_metric_type((GModule)module, module_metrics[i]);
    }
  }
}

void free_persisted_data(void) { free(persisted_dates); }
